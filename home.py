...
chat_templates = ['code_deepseek', 'chat_mistral', 'chat_mixtral_base', 'chat_redguard', 'chat_artbell', 'chat_halflife']
...
if st.session_state['approved_login']:
    ...
    with tab1:
        ...
        st.session_state['enable_microphone'] = st.toggle(':orange[enable microphone]', value=st.session_state['enable_microphone'], disabled=llmonaid.check_user_type())
        st.session_state['enable_voice'] = st.toggle(':orange[enable] :green[xttsv2 tts]', value=st.session_state['enable_voice'], disabled=llmonaid.check_user_type())
        st.session_state['enable_voice_melo'] = st.toggle(':orange[enable] :blue[melotts]', value=st.session_state['enable_voice_melo'])
        st.session_state['user_audio_length'] = st.text_input(":orange[microphone rec time(sec)]", value=st.session_state['user_audio_length'], disabled=llmonaid.check_user_type())
        ...
        if st.session_state['enable_voice']:
            ...
            for key, value in voice_box_dict.items():
                if value == st.session_state['voice_select']:
                    set_voice_index = key
            st.session_state['voice_select'] = st.selectbox(':orange[voice file]', voice_box_data, index=set_voice_index, disabled=llmonaid.check_user_type())
            st.audio(f"./voices/{st.session_state['voice_select']}")
            st.session_state['stream_chunk_size'] = st.text_input(':orange[tts stream chunk size]', value=int(st.session_state['stream_chunk_size']), disabled=llmonaid.check_user_type())
            st.session_state['chunk_pre_buffer'] = st.text_input(':orange[tts chunk buffers]', value=int(st.session_state['chunk_pre_buffer']), disabled=llmonaid.check_user_type())
    ...
    with tab2:
        ...
        if st.session_state['model_select'] == "deepseek-coder-33b-instruct.Q5_K_M.gguf":
            st.session_state['gpu_layer_count'] = 36         
    ...
    with tab3:
        ...
        st.session_state['enable_vision'] = st.toggle(':orange[enable] :violet[moon]:orange[dream2]', value=st.session_state['enable_vision'])
